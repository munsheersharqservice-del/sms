...
export const LoginPage: React.FC = () => {
  const { login, signup, users, sendOtp, verifyOtp, resetPassword, connectGoogle } = useApp();
                ...
                <form onSubmit={handleLoginSubmit} className="space-y-4">
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        placeholder="Enter email, username, or select below"
                        className="w-full pl-10 pr-4 py-2.5 bg-[#F8F9FA] border border-slate-200 rounded-xl text-[#212529] text-xs sm:text-sm font-semibold focus:bg-white focus:border-[#1D3557] focus:ring-2 focus:ring-[#1D3557]/15 focus:outline-hidden transition-all"
                      />
                      <datalist id="engineers-list">
                        {users.map((u) => (
                          <option key={u.id} value={u.email}>
                            {u.name} — {u.role || 'Service Engineer'}
                          </option>
                        ))}
                      </datalist>
                    </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        placeholder="Default PIN: 123 (or master 2277)"
                    ...
                    <div className="flex items-center justify-between text-[11px] text-slate-500 mt-1 px-1">
                      <span>Default PIN: <code className="font-mono font-bold text-[#1D3557]">123</code> (or <code className="font-mono font-bold text-[#1D3557]">108</code>)</span>
                      <span>Master Code: <code className="font-mono font-bold text-[#1D3557]">2277</code></span>
                    </div>
                  </div>
                  ...
                  <div className="relative my-3">
                    ...
                    <div className="relative flex justify-center text-[10px] uppercase">
                      <span className="bg-white px-2 text-slate-400 font-bold tracking-wider">Or Sign In With</span>
                    </div>
                  </div>
                  ...
                  <button
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        if (ok) {
                          setSuccessMsg('Google authentication successful! Redirecting to portal...');
                        } else {
                    ...
                    className="w-full py-2.5 px-4 bg-white hover:bg-slate-50 text-slate-700 border border-slate-300 rounded-xl font-bold text-xs sm:text-sm flex items-center justify-center space-x-2.5 transition-all shadow-xs cursor-pointer disabled:opacity-50"
                  >
                  ...
                  <div className="pt-3 border-t border-slate-100">
                    <p className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2 flex items-center justify-between">
                      <span>Quick Engineer 1-Click Access:</span>
                      <span className="text-[10px] text-slate-400 font-normal">Select your profile</span>
                    </p>
                      ...
                      {users
                        ...
                        .map((eng) => (
                          <button
                            ...
                            type="button"
                            onClick={() => {
                              const pass = eng.password || '123';
                              setLoginIdentifier(eng.email || eng.name);
                              setLoginPassword(pass);
                              setErrorMsg('');
                              setIsLoading(true);
                              setTimeout(() => {
                                const ok = login(eng.email || eng.name, pass, rememberMe);
                                setIsLoading(false);
                                if (!ok) {
                                  setErrorMsg(`Could not log in as ${eng.name}`);
                                }
                              }, 150);
                            }}
                            className="px-2.5 py-1.5 bg-slate-100 hover:bg-[#1D3557] hover:text-white rounded-lg text-[11px] font-semibold text-slate-700 transition-colors flex items-center space-x-1.5 cursor-pointer shadow-2xs"
                            title={`Click to instantly log in as ${eng.name}`}
                          >
